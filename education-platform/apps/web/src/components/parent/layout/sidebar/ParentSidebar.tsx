...
export const navItems = [
    ...
    { id: "5", href: '/parent/dashboard/notifications', icon: Bell, label: 'Thông báo', badge: 3 },
    { id: "6", href: '/parent/dashboard/ai-grader', icon: Bell, label: 'Máy chấm điểm AI', badge: 3 },
];


export function ParentSidebar({ pathname, sidebarOpen }: ParentSidebarProps) {
    const { selectedMemberId, getMemberById, parents, logout } = useApp();
            ...
            <div className={`${sidebarOpen ? 'w-56' : 'w-14'} flex flex-col transition-all duration-200 shrink-0 h-full`} style={{ background: '#FFFFFF', borderRight: `1px solid ${COLORS.BORDER}` }}>
                    ...
                    <div className="mx-3 mt-3 p-2.5 rounded-2xl" style={{ background: COLORS.MINT_BG }}>
                        <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: '#A7F3D0', color: COLORS.MINT_DARK }}>
                                {getMemberById(selectedMemberId)?.name[0]}
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-bold truncate" style={{ color: COLORS.MINT_DARK }}>{getMemberById(selectedMemberId)?.name}</p>
                                <p className="text-xs" style={{ color: COLORS.MINT }}>
                                    Lớp {getMemberById(selectedMemberId)?.grade}
                                </p>
                            </div>
                            <button onClick={() => { router.push("/parent/dashboard/members"); logout(); }} style={{ color: COLORS.MINT_DARK }} onMouseEnter={e => (e.currentTarget.style.color = COLORS.MINT)} onMouseLeave={e => (e.currentTarget.style.color = COLORS.MINT_DARK)}>
                                <X className="w-3 h-3" />
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex-1 flex flex-col justify-between">
                    ...
                    <nav className="flex-1 p-2.5 space-y-0.5 mt-2">
                        {navItems.map(item => {
                            const isDisabled = item.requiresMember && (!selectedMemberId || !getMemberById(selectedMemberId));
                    ...
                    <div className="p-2.5" style={{ borderTop: `1px solid ${COLORS.BORDER}` }}>
                        <div className="flex items-center gap-2.5 px-2 py-2 rounded-2xl" style={{ cursor: 'default' }}>
                            ...
                            {sidebarOpen && (
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-bold truncate" style={{ color: COLORS.TEXT }}>{parent.name}</p>
                                    <p className="text-xs truncate" style={{ color: COLORS.MUTED }}>{parent.email}</p>
                                </div>
                            )}
